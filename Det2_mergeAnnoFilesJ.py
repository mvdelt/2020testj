...
def merge_two_anno_obj(anno1, anno2):
    ...
    #### i. 2020.10.25.일욜. 무슨이유인지 coco-annotator 계정들과 기존 어노테이션 기록들 다 날아가서, 새로계정만들고 기존 어노json파일 임포트햇더니 이미지id 도 1부터 시작하고, 카테고리id도 기존의 1이 아닌 어쩌다보니 3으로 돼서, 기존 어노json파일들과 머지시키려면 수정 필요. 
    #### i. 아마 예전에 이거 작성할땐, 카테고리가 여러개일경우도 생각했었나본데, *일단 카테고리 1개인경우에 대해서만* 하자.
    categories_same_flag = True
    for i in anno1_category_list:
        if i not in anno2_category_list:
            ...
            categories_same_flag = False
    if not categories_same_flag:
        print('j) @@@@@ 카테고리가 불일치하는게 있음!! 아마 2020.10.25.에 계정 다 날아간 이후에 어노테이션 해준 json파일을 머지시키려나본데, 그렇다고 가정하고 진행하겠음!!!!!')
        print('j) @@@@@ 즉, 카테고리 id 를 기존의 값인 1로 다 통일시킬거임!!!')
        #### i. 바로위에서 for문 돌린거는 카테고리가 여러개일때를 대비했던것같은데, 일단 걍 카테고리 1개라고 치고 하자(카테고리 리스트에 카테고리 1개뿐).
        for cateList in [anno1_category_list, anno2_category_list]:
            if len(cateList)!=1: raise ValueError('j) 카테고리 리스트의 원소(카테고리) 갯수가 1이 아님!!!!!!')
        for cate in [anno1_category_list[0], anno2_category_list[0]]:
            #### i. 하나의 카테고리는 딕셔너리로 되어있음. 어노테이션json파일 보면 알지.
            print('j) categori id: ', cate["id"])
            cate["id"] = 1
        #### i. 각 이미지 정보의 "category_ids" 리스트의 원소도 1하나로 해줌.(원래 1하나엿는데 지금 이상하게돼서 새로만든 어노json파일에선 아이디값이 3으로되어있으니..)
        for anno in [anno1, anno2]:
            for imginfo in anno["images"]:
                imginfo["category_ids"] = [1]
    ...
    #### i. 2020.10.26.새벽. 추가한부분. - 각 어노테이션 정보 (딕셔너리) 의 category_id 의 값을 1로 걍 통일.
    for anno in [anno1, anno2]:
        for annoinfo in anno["annotations"]:
            annoinfo["category_id"]=1  
...
# merge_anno_files(r"C:\Users\starriet\Desktop\pa_keypointj_lower_merged", r"C:\Users\starriet\Desktop\pa_keypointj_lower_merged\pa_kp_anno_lower_merged_until3-total103over362.json")
merge_anno_files(r"C:\Users\starriet\Desktop\pa_keypointj_lower_merged", r"C:\Users\starriet\Desktop\pa_keypointj_lower_merged\pa_kp_anno_lower_merged_until3-total143over362.json")
