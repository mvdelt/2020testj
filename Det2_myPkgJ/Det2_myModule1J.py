...
class KRCNNConvDeconvUpsampleHead_heatmapj(KRCNNConvDeconvUpsampleHead):
    ...
    def _put_kplogits_into_instancesj(self, pred_kp_logits, pred_instances):
        ...
        pred_kp_logits_splited = pred_kp_logits.split(
            num_objects_per_image_list, dim=0)
...
class Visualizer_kplogitsj(Visualizer):
    ...
    def draw_kplogitsj(self, predictions):
        ...
        # i. if predictions.has("pred_boxes") else None 이거 왜 안붙여줬냐면, 만약에 특정 인풋이미지(PA이미지)에서 디텍션된 물체(임플픽스쳐)가 없다고 해도, predictions.pred_boxes 등에는 비어있는 텐서가 들어갈거기때문에 predictions.has("pred_boxes") 의 값은 어차피 True 일 거거든(일단 내가 코드 훑어보기론 그러함). 일단 걍 그래서 if~ 는 빼줘봄.
        ...
        for kplogits_per_obj, box in zip(kplogits_per_image, boxes):
            ...
            self.output.boxCoordsTuple_listj.append((int(round(box[0].item())), int(
                round(box[1].item())), int(round(box[2].item())), int(round(box[3].item()))))

            # i. 키포인트가 6개니까 키포인트로짓(히트맵)도 6개인데 하나의 box에 그려줘야하니까 걍 다 더해서 한 box에 6개의 로짓분포들이 다 보이도록 해줘보자.
            # i. torch tensor.
            kplogits_per_obj_summed = torch.sum(kplogits_per_obj, 0)
            # i. numpy array.
            kplogits_per_obj_summed_arr = kplogits_per_obj_summed.numpy()
            ...
            boxSize = (int(round(box[2].item()-box[0].item())),
                       int(round(box[3].item()-box[1].item())))
            ...
            kplogits_per_obj_summed_arr = cv2.resize(
                kplogits_per_obj_summed_arr, dsize=boxSize, interpolation=cv2.INTER_CUBIC)
            ...
            # i. 오! extent 설정해주니 이미지 좌표 설정 되네!! interpolation 옵션값 골라주는대로 인터폴레이션 되고(이미지 확대/축소시에).
            kplogitsPerObj_AxesImage = self.output.ax.imshow(kplogits_per_obj_summed_arr, alpha=0.4, extent=(
                box[0].item(), box[2].item(), box[3].item(), box[1].item()), interpolation='sinc')
    ...
    def draw_instance_predictions(self, predictions):
        ...
        labels = self._create_text_labelsj(
        ...
        self.overlay_instancesj(
            ...
            labels=labels,
            keypoints=keypoints,
            # i. 요거 추가해줌. 시각화결과 그려줄때 키포인트 로짓 정보를 활용해보려고(예를들어 키포인트마다 로짓 값 보여준다든지).
            kplogits_per_image=kplogits_per_image,
    ...
    def show_kplogitsj(self, kplogits_per_image):
        ...
        assert hasattr(
            self.output, 'heatmapaddedCroppedArr_listj'), "j) this method(show_kplogitsj) should be called AFTER set 'heatmapaddedCroppedArr_listj' in the VisImage object!!"
        # i. VisImage 객체에 내가 저장해놨던 box크롭된 이미지어레이들 리스트를 사용.
        croppedArr_list = self.output.heatmapaddedCroppedArr_listj
        ...
        # i. 일단 교수님도 봐야해서 내 어노테이션방식이랑 좌우 다르게 했음.(키포인트인덱스->키포인트명칭 맵핑용 딕셔너리임.)
        kpIdx2kpName = {0: "Lt b. level", 1: "Rt b. level",
                        2: "Lt apex", 3: "Rt apex", 4: "Lt top", 5: "Rt top"}
        kpPlotRGBA_arr_list = []
        for objIdx, (kplogit_per_object, croppedArr) in enumerate(zip(kplogits_per_image, croppedArr_list)):
            ...
            kp_fig = mpl.figure.Figure(
                constrained_layout=True, figsize=(6.4, 4.8), dpi=100)
            ...
            # i. 프레딕션된 키포인트로짓 순서가 COCO형식으로 어노테이션해준 그 어노테이션 순서대로였었지 아마. 지금내경우엔 bone right, bone left, apex right, apex left, top right, top left 순서.(좌우는 내방식.)
            kp_ax_list = [kp_ax4, kp_ax3, kp_ax2, kp_ax1, kp_ax6, kp_ax5]

            kp_fig.suptitle(
                "Fixture{}/{} heatmaps".format(objIdx+1, num_objs_per_img), fontsize=14)
            ...
            for kpIdx, (kplogit_per_kp, kp_ax) in enumerate(zip(kplogit_per_object, kp_ax_list)):
                ...
                kp_ax.set_title('{}'.format(kpIdx2kpName[kpIdx]), fontdict={
            ...
            kp_fig.colorbar(mpl.cm.ScalarMappable(
                norm=None, cmap=None), cax=None, ax=kp_ax_list, use_gridspec=True)
            kp_canvas = FigureCanvasAgg(kp_fig)
            kp_canvas.draw()
            # i. canvas.buffer_rgba() 가 결국 canvas.renderer.buffer_rgba() 를 호출해서 리턴하는거임. 결국 같은것.
            buf = kp_canvas.buffer_rgba()
    ...
    def overlay_instancesj(
        ...
        *,
        ...
        if assigned_colors is None:
            assigned_colors = [random_color(
                rgb=True, maximum=1) for _ in range(num_instances)]
        if num_instances == 0:
            return self.output
        ...
        if keypoints is not None:
            for keypoints_per_instance, kplogits_per_instance in zip(keypoints, kplogits_per_image):
                self.draw_and_connect_keypointsj(
                    keypoints_per_instance, kplogits_per_instance)

        return self.output

    # i. 인풋인자에 내가 키포인트 로짓 추가함.
    def draw_and_connect_keypointsj(self, keypoints, kplogits_per_instance):
        """
        ...
        for idx, (keypoint, kplogits_for_onekp) in enumerate(zip(keypoints, kplogits_per_instance)):
            ...
            if prob > _KEYPOINT_THRESHOLD:
                if idx == 0 or idx == 1:  # i. 키포인트가 bone right, bone left 인 경우.
                    # i. draw_circle 메서드에 radius, fill, zorder 넣어줫음. 라인두께도 설정가능하게 빼줬음.
                    self.draw_circle((x, y), color=_RED, radius=14,
                                     linewidthj=4.5, fill=False, zorder=20)
                else:  # i. 그 외의 키포인트들.
                    self.draw_circle((x, y), color=(
                        255/255, 0/255, 0/255, 1), radius=10.5, fill=True, zorder=15)
                if keypoint_names:
                ...
                self.draw_text(
                    ...
                    # 출력할 텍스트 내용. - 일단 해당 키포인트(하나)의 로짓값들중 최대값을 출력해줘봄. ## i. 2020.06.21.추가) prob 값도 출력해봄. 뭔지 좀 보게.
                    '{}/{}'.format(round(float(kplogits_for_onekp.max()),
                                         1), round(float(prob), 2)),
                    (x+10, y-9),  # 포지션. 일단출력먼저하고 나중에 수정.
                    font_size=self._default_font_size * 1.4,  # 일단걍해보고 나중에 수정.
                    # 일단 "g"(green일거임) 넣어보고 나중에 수정.->RGBA값으로해줘봄.튜플.
                    color=(150/255, 250/255, 50/255, 0.8),
                    # RGBA, 0~1사이의값들로구성. -> 아.근데 알파값은 어차피 요바로아래서정해주는걸로 적용되나봄. 걍 RGB(3가지값)튜플로 해줘봄.
                    facecolorj=(0/255, 0/255, 0/255),
                    face_alphaj=0,
        ...
        osseo_fixture_len = math.sqrt(
            (apex_x-bonelvl_x)**2 + (apex_y-bonelvl_y)**2)
        # 본컨택된픽스쳐부위의 비율.
        osseo_fixture_pct = round((osseo_fixture_len/tot_fixture_len)*100, 1)
        # 본컨택된픽스쳐부위의 비율을 이용해서 클래스 분류.
        boneLossCls = 'Normal' if osseo_fixture_pct >= 90 else 'Early' if osseo_fixture_pct >= 75 else 'Moderate' if osseo_fixture_pct >= 50 else 'Severe'
        # 3) 본컨택된픽스쳐부위의 비율, 분류결과 시각화 출력.
        self.draw_text(  # 비율 출력(예: 73.55% 이런식).
            'MBL {}%'.format(round(100-osseo_fixture_pct, 1)),  # 출력할 텍스트 내용.
            # 포지션. 일단 픽스쳐의 한가운데로 잡아봄. 일단출력먼저하고 나중에 수정.
            ((apex_x+top_x)/2, (apex_y+top_y)/2 + 15),
            # 기본값에 3곱해줘봄. 일단걍해보고 나중에 수정.
            font_size=self._default_font_size * 2.5,
            # 일단 "g"(green일거임) 넣어보고 나중에 수정.->RGBA값으로해줘봄.튜플.
            color=(127/255, 0/255, 255/255, 1.0),
            # RGBA, 0~1사이의값들로구성. -> 아.근데 알파값은 어차피 요바로아래서정해주는걸로 적용되나봄. 걍 RGB(3가지값)튜플로 해줘봄.
            facecolorj=(0/255, 0/255, 0/255),
            face_alphaj=0,
        ...
        self.draw_text(  # 분류결과 출력(예: Early 이런식).
            ...
            # 포지션. 일단 픽스쳐의 한가운데로 잡아봄. 일단출력먼저하고 나중에 수정.
            ((apex_x+top_x)/2, (apex_y+top_y)/2 - 50),
            # 기본값에 3곱해줘봄. 일단걍해보고 나중에 수정.
            font_size=self._default_font_size * 3,
            # 일단 "g"(green일거임) 넣어보고 나중에 수정.->RGBA값으로해줘봄.튜플.
            color=(0/255, 0/255, 204/255, 1.0),
            # RGBA, 0~1사이의값들로구성. -> 아.근데 알파값은 어차피 요바로아래서정해주는걸로 적용되나봄. 걍 RGB(3가지값)튜플로 해줘봄.
            facecolorj=(0/255, 0/255, 0/255),
            face_alphaj=0,
            horizontal_alignment="center",  # see 'matplotlib.text.Text'
        ...
        if self.metadata.get("keypoint_connection_rules"):
            for kp0, kp1, color in self.metadata.keypoint_connection_rules:
                if kp0 in visible and kp1 in visible:
                    ...
                    # i. linestyle, linewidth, zorder 넣어줌. linewidth=None 이엇는데 2로 바꿔줘봄.
                    self.draw_line([x0, x1], [y0, y1], color=color,
                                   linestyle="-", linewidth=7, zorder=5)
        return self.output
    ...
    def draw_text(
        ...
        **kwargsj
        ...
        self.output.ax.text(
            ...
            family="sans-serif",
            bbox={"facecolor": facecolorj, "alpha": face_alphaj,
    ...
    # i. fill, zorder 넣어줌.
    def draw_circle(self, circle_coord, color, radius=3, linewidthj=None, fill=True, zorder=20):
        """
        Args:
        ...
        self.output.ax.add_patch(
            ...
            mpl.patches.Circle(circle_coord, radius=radius,
                               linewidth=linewidthj, fill=fill, color=color, zorder=zorder)
        )
        return self.output

    # i. zorder 넣어줫음.
    def draw_line(self, x_data, y_data, color, linestyle="-", linewidth=None, zorder=5):
        """
        Args:
    ...
    def _create_text_labelsj(self, classes, scores, class_names):
        ...
        if classes is not None and class_names is not None and len(class_names) >= 1:
            ...
            # i. 요정도 형변환(숫자->스트링)정도는 파이썬에선 자동으로 해줌.
            labels = ["{}/{}) {}".format(idx+1, num_objs_per_img, label)
...
# i. <-일단 딱 요 한줄만 넣어서 함수화해봄. predictor, Visualizer_kplogitsj 등도 인풋인자로 받아야하나? 일단 요렇게만 해서 해보자.
def visualizeCodeFct_j(val_imgs_pathj):

    # i. 드뎌 내가 수정해준 Visualizer_kplogitsj 클래스 사용!!
    # %cd /content
    val_img_filename_list = os.listdir(val_imgs_pathj)
    # i. 폴더에 어노테이션json파일 있어서, 그놈은 제외하려고 넣어준 코드.
    val_img_filename_list = [
        fname for fname in val_img_filename_list if '.json' not in fname]
    ...
    for test_img_filename in val_img_filename_list[:20]:
        testimg_arr = cv2.imread(os.path.join(
            val_imgs_pathj, test_img_filename))
        ...
        visImage = visualizer.draw_instance_predictions(
            pred_pa_kp_upper["instances"].to("cpu"))
        pred_drawed_testimg_arr = visImage.get_image()
        # i. 이미지 넘 커서 리사이즈. Visualizer객체 이니셜라이즈할때 scale 정해주면 되는데, 지금 scale값 1인상태로 히트맵추가등 다 해줘버려서, 걍 이렇게 리사이즈해줌 일단.
        pred_drawed_testimg_arr_resized = cv2.resize(pred_drawed_testimg_arr, tuple(int(round(
            0.5*i)) for i in pred_drawed_testimg_arr.shape[:2][::-1]), interpolation=cv2.INTER_AREA)
        # i. 디텍션한 bbox, keypoints 그려준 이미지어레이 보여줌.@@@@@@@@@@@@@@@@@@(출력1)
        cv2_imshow(pred_drawed_testimg_arr_resized[:, :, ::-1])
        ...
        # i. 이미지 넘 커서 리사이즈. Visualizer객체 이니셜라이즈할때 scale 정해주면 되는데, 지금 scale값 1인상태로 히트맵추가등 다 해줘버려서, 걍 이렇게 리사이즈해줌 일단.
        pred_drawed_testimg_heatmapadded_arr_resized = cv2.resize(pred_drawed_testimg_heatmapadded_arr, tuple(int(
            round(0.5*i)) for i in pred_drawed_testimg_heatmapadded_arr.shape[:2][::-1]), interpolation=cv2.INTER_AREA)
        # i. 위의결과에 추가로 히트맵까지 더해준 이미지어레이 보여줌.@@@@@@@@@@@@@@@@@@(출력2)
        cv2_imshow(pred_drawed_testimg_heatmapadded_arr_resized[:, :, ::-1])
        ...
        # i. box 크롭된 이미지어레이를 담을 리스트 만듦.
        visImage.heatmapaddedCroppedArr_listj = []
        # i. 위에서 호출한 draw_kplogitsj 메서드에서 visImage 객체에 boxCoordsTuple_listj 를 만들어놨음.
        for i in visImage.boxCoordsTuple_listj:
        croppedArrj = pred_drawed_testimg_heatmapadded_arr[i[1]
            :i[3], i[0]:i[2]]
        ...
        visualizer.show_kplogitsj(
            pred_pa_kp_upper["instances"].to("cpu").pred_keypoint_logits)
        num_detectedObjs = len(visImage.kpPlotRGBA_arr_listj)
        # i. 위에서 호출한 show_kplogitsj 메서드에서 visImage 객체에 kpPlotRGBA_arr_listj 를 만들어놨음.
        for idx, i in enumerate(visImage.kpPlotRGBA_arr_listj):
            # i. 히트맵들 플롯팅해서 정리한거 보여줌.@@@@@@@@@@@@@@@@@@(출력3)
        cv2_imshow(i[:, :, [2, 1, 0, 3]])
